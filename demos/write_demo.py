...
from demos.style import (DIM, DOCS, GROUND, HORSE, L4C, L5C, PLANET, STAR,
                         optimize_gif)
from orbital import memory, theory, write

T = memory.PERIOD


def timing():
    """The datasheet, generated by the PRODUCTION write path (write.scan_delays
    = the exact write_bit pipeline at each delay), so guard bands and write
    windows are the real thing, not a looser-tolerance approximation."""
    delays = np.arange(0, 40, 3)
    results = write.scan_delays([d * T for d in delays])
    outcomes = [{"1": "L4", "0": "L5"}.get(r["bit"], "erased")
                for r in results]
...
def gif(frames=150):
    ...
    # read() windows by TIME, so coarse animation sampling is fine
    bit, center, amp = write.read(run, window_orbits=40)
